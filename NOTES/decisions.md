...
## DEC-025 — Code review: 10 demands + 15 suggestions implemented
**Date**: 2026-04-15
**Decision**: Full code review (Apple Head of Product Development standard) rated the codebase 7.0/10. All 10 demands and 15 suggestions implemented in a single pass. PDF report: `SaveThisVideo_v1.2_Code_Review.pdf`.
**Demands resolved**: D1 cross-thread `_last_saved_path` (→ closure-local + after() arg), D2 partial file cleanup (→ `_cleanup_partial`), D3+D4 TOCTOU + probe failure overwrite (→ `nooverwrites: True`), D5 osascript injection (→ ASCII sanitize + stdin pipe), D6 probe hang (→ `socket_timeout: 15` + cancel check), D7 playlist deadlock (→ short-timeout loop + `_playlist_done`), D8 stale `_tick` (→ `_downloading` guard), D9 hook error status (→ `elif "error"` + log), D10 CLAUDE.md vs build.sh signing (→ rewrote build section).
**Suggestions resolved**: S1 time validation, S2 URL truncate, S3 paste focus, S4 disable controls during download, S5 captured `save_dir`, S6 loop cap, S7-S8 requirements comments, S9 stale spec deletion, S10 filename limitation documented, S11 writable dir check, S12 force-close warning, S13 probe error logging, S14 `__all__`, S15 Escape keybinding.
**Architectural changes**: Progress hook moved from standalone `_hook(self, d)` method to a closure inside `_worker()` — eliminates cross-thread shared state for `saved_path`. `_input_widgets` list + `_set_controls_enabled()` added for mid-download control disabling. `_playlist_done` event added to `__init__` for deadlock prevention.
